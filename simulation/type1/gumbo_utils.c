...
#define THRESHOLD           10
#define WITHIN_THRESHOLD(x) (x < THRESHOLD || x + THRESHOLD > sizeof(gumbo_rev_t))
...
BOOL is_opcode_packet(const char *msg)
{
  ...
  if (  opcode == QUERY_OPCODE || opcode == CONFIRM_OPCODE ||
        opcode == STOP_OPCODE || opcode == CONTINUE_OPCODE  )
...
int log_and_save(const char *message)
{
  ...
  struct gumbo_node_data *ptr_old_data;
  struct gumbo_node_data *ptr_new_data;
  
  extract_packet_data(message, &buffer);
  ptr_old_data = find_entry(buffer.addr);
  
  if (ptr_old_data == NULL || compare_revisions(buffer.rev, ptr_old_data->rev) > 0) {
    
    if (ptr_old_data)
      list_remove(gumbo_node_entries, ptr_old_data);

    /* allocate new space and push to the list */      
    ptr_new_data = memb_alloc(&gumbo_mem_pool);
    memcpy(ptr_new_data, &buffer, sizeof(struct gumbo_node_data));
    list_push(gumbo_node_entries, ptr_new_data);
  }
}
...
{  
  ...
  else if (WITHIN_THRESHOLD(r1) && WITHIN_THRESHOLD(r2))
    return r2 - r1;
...
static struct gumbo_node_data *internal_itr;
